...
def add(data):
    cursor = db.cursor()
    sql="""INSERT INTO member_sma (nama,alamat,paket,kelas,asal_sekolah,no_hp) VALUES (%s,%s,%s,%s,%s,%s)"""
...
def edit(data):
    cursor = db.cursor()
    sql="""UPDATE member_sma SET nama=%s,alamat=%s,paket=%s,kelas=%s,asal_sekolah=%s,no_hp=%s WHERE id=%s"""
    cursor.execute(sql,data)
    db.commit()
...
def delete(data):
    cursor=db.cursor()
    sql="""DELETE FROM member_sma WHERE id=%s"""
    cursor.execute(sql,data)
    db.commit()
